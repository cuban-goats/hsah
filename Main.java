...
public class Main {
  ...
  public static char[][] prepare_size(String input, Boolean print) {
    ...
    char[][] base_chars = init_values(false, base, false);
    ...
    while (input_string.length() > 128) {
      ...
      for (int i = 0; i <= ((static_input_string.length() / 128)); i++) {
        ...
        char[][] cut_input_chars = init_values(true, cut_input, false);
  ...
  public static char[][] init_values(Boolean convert, String data, Boolean print) {
    String output = data;

    if (convert == true) {
      output = binary_conversion(data);
    }
    if (print == true) {
      System.out.println("\nunadjusted_string:\n" + output + "\n");
    }
  ...
  public static void hsah(String data, Boolean print_inputs, Boolean print_xor, Boolean print_shift,
    ...
    char[][] result = prepare_size(
        "",
        true);
    ...
    char[][] input_one = init_values(true, data, print_inputs);

    // static hsahing datasets
    char[][] input_two = init_values(true, "823b4erhdsa8ufnb", print_inputs);
    char[][] input_three = init_values(true, "lköwrienheß", print_inputs);
    char[][] input_four = init_values(true, "jkasdhh2ndin", print_inputs);
    ...
    int[] input_numeric_value_two = decimal_conversion(init_values(true, "ds7basdjfgh7d", print_inputs),
        print_decimals);
    int[] input_numeric_value_three = decimal_conversion(init_values(true, "982hejhdsabfn", print_inputs),
        print_decimals);
    int[] input_numeric_value_four = decimal_conversion(init_values(true, "897h32jk1n0d*", print_inputs),
        print_decimals);
    ...
    char[][] binary_values = init_values(true, string_mod_values, print_inputs);
  };
}
